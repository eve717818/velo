...
`/notes` 已具备可用的本地优先基础闭环：收件箱创建、Markdown 标题与正文、自动保存及重载恢复、任意节点正文、子笔记、同级笔记、移动、子树软删除与恢复，以及单文件 Markdown 下载。浏览器验收没有发现需要修改产品代码的问题。

Markdown 阅读模式不会执行原始 HTML/脚本；Markdown 远程图片会被替换为“已阻止加载”提示，验收监听到的远程图片请求为 0。链接继续由渲染器执行安全 URL 转换，并在新窗口打开时带 `noopener noreferrer`。
...
| 视口宽度 | 布局 | 200% 文字 | 全局横向溢出 | 边界记录 |
| ---: | --- | --- | --- | --- |
| 390 | 手机正文 + 目录抽屉 | 通过 | 无 | content / editor / drawer / tree node |
| 402 | 手机正文 + 目录抽屉 | 通过 | 无 | content / editor / drawer / tree node |
| 768 | 双栏平板 | 通过 | 无 | content / editor / directory / tree node |
| 834 | 双栏平板 | 通过 | 无 | content / editor / directory / tree node |
| 1024 | 双栏平板/桌面 | 通过 | 无 | content / editor / directory / tree node |
| 1366 | 桌面 | 通过 | 无 | content / editor / directory / tree node |

每个宽度的边界数据由 Playwright 作为 JSON 附件保存到当次测试结果。稳定截图位于项目的忽略目录 `staging/notes-foundation/`：
...
- `http://127.0.0.1:4176/` 可访问，返回 HTML 与最新 `dist/index.html` 完全一致；当前入口指纹为 `index-Bjue-hUj.js` 与 `index-BgjmH4B7.css`。
