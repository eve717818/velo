...
- 简化版 Obsidian 笔记工作台：文件夹与笔记分离、无限层级树状目录、完整路径、新建/移动/重命名、子树回收恢复与 Markdown 导出
...
- 左侧“目录”按文件夹与笔记组织知识树，可无限细分；每次新建与重命名都会显示完整父级路径，长标题在树中自动省略
- 手机端目录使用左侧抽屉；选择或新建文件夹/笔记后自动收起，并在右侧显示完整路径与文件夹操作或笔记正文
- “每日灵感”是独立固定入口：用万年历选择任意日期，在空白浅米黄卡片中直接书写；内容自动保存并以日历圆点标记，清空后不保留空记录
- 普通笔记与每日灵感都在切换前完成保存；写入失败的草稿留在当前设备，多页面修订冲突不会静默覆盖新版本
- 回收站保留被删除的文件夹、笔记及原有层级，当前阶段支持恢复；“导出 Markdown”会先等待当前草稿成功保存
