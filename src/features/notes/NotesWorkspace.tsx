...
type Area = "all" | "trash"
type NodeEditorState = { mode: "create"; type: "folder" | "note"; parentId: string | null } | { mode: "rename"; node: KnowledgeNode }
...
export function NotesWorkspace({ db, services }: NotesWorkspaceProps) {
  ...
  const selectedNodeId = urlNoteId ?? selectedFolderId
  ...
  async function selectNode(node: KnowledgeNode) {
    ...
    setSelectedFolderId(node.type === "folder" ? node.id : null)
  ...
  function startCreation(selection: NewKnowledgeNodeSelection) { setNodeEditor({ mode: "create", ...selection }) }
  ...
  async function commitNodeTitle(title: string) {
    ...
    if (nodeEditor.mode === "rename") {
      ...
      setSelectedFolderId(renamed.type === "folder" ? renamed.id : null)
    } else {
      ...
      setSelectedFolderId(created.type === "folder" ? created.id : null)
      const next = new URLSearchParams(params)
  ...
  async function doTrash() {
    ...
    else setSelectedFolderId(selected.id)
    setParams(next)
  }
  ...
  const directory = (
    <div className={styles.directoryBody}>
      ...
      <NoteTree expandedIds={expandedIds} nodes={visibleNodes} onSelect={selectNode} onToggle={toggleFolder} selectedId={selectedNodeId} />
    </div>
  )
  const editorLabel = nodeEditor?.mode === "rename" ? `${nodeEditor.node.type === "folder" ? "文件夹" : "笔记"}名称` : nodeEditor?.type === "folder" ? "文件夹名称" : "笔记名称"
  const editorInitialValue = nodeEditor?.mode === "rename" ? nodeEditor.node.title : ""
    ...
    <main className={styles.page}>
      <header className={styles.workspaceHeader}>
        ...
        <div className={styles.headerActions}>
          ...
          <NewKnowledgeNodeMenu onSelect={startCreation} />
        </div>
      </header>
      ...
      <div className={styles.workspace} style={{ "--notes-sidebar-width": `${sidebarWidth}px` } as React.CSSProperties}>
        ...
        <section className={styles.contentPane} aria-label="笔记内容">
          {nodeEditor ? <div className={styles.emptyState}><span><FolderOpen aria-hidden="true" /></span><h2>{nodeEditor.mode === "rename" ? "重命名" : `新建${nodeEditor.type === "folder" ? "文件夹" : "笔记"}`}</h2><TreeNodeEditor ariaLabel={editorLabel} initialValue={editorInitialValue} onCancel={() => setNodeEditor(null)} onCommit={commitNodeTitle} /></div> : selected?.deletedAt !== undefined ? <div className={styles.deletedState}><Trash2 aria-hidden="true" /><h2>{selected.title}</h2><p>{selected.type === "folder" ? "该文件夹在回收站中，恢复后将带回所有子文件夹和笔记。" : "这篇笔记在回收站中，恢复后即可继续编辑。"}</p><button onClick={() => void doRestore()} type="button">恢复{selected.type === "folder" ? "文件夹" : "笔记"}</button></div> : selected?.type === "folder" ? <div className={styles.emptyState}><span><FolderOpen aria-hidden="true" /></span><h2>{selected.title}</h2><p>{breadcrumbs.map((node) => node.title).join(" / ")} · {folderChildCount} 个子节点</p><div className={styles.folderActions}><button onClick={() => startCreation({ type: "folder", parentId: selected.id })} type="button">新建子文件夹</button><button onClick={() => startCreation({ type: "note", parentId: selected.id })} type="button">新建笔记</button><button aria-label={`节点操作：${selected.title}`} onClick={(event) => { setActionReturnFocus(event.currentTarget); setActionsOpen(true) }} type="button"><MoreHorizontal aria-hidden="true" />操作</button></div></div> : selected ? <><div className={styles.noteToolbar}><nav aria-label="当前笔记路径" className={styles.breadcrumbs}>{breadcrumbs.map((node, index) => <span key={node.id}>{index ? <i aria-hidden="true">/</i> : null}<button onClick={() => void selectNode(node)} type="button">{node.title}</button></span>)}</nav><div className={styles.noteActions}><button onClick={() => void doExport()} type="button"><Download aria-hidden="true" />导出 Markdown</button><button aria-label={`节点操作：${selected.title}`} onClick={(event) => { setActionReturnFocus(event.currentTarget); setActionsOpen(true) }} type="button"><MoreHorizontal aria-hidden="true" /></button></div></div><NoteEditor db={db} key={selected.id} nodeId={selected.id} onSaved={() => undefined} ref={editorRef} saveNote={api.saveNote} /></> : <div className={styles.emptyState}><span><NotebookPen aria-hidden="true" /></span><p>{area === "trash" ? "回收站是空的" : "从第一个文件夹或笔记开始建立你的知识目录"}</p>{area !== "trash" ? <button onClick={() => startCreation({ type: "note", parentId: null })} type="button"><Plus aria-hidden="true" />开始记录</button> : null}</div>}
        </section>
      </div>
      ...
      {selected && selected.deletedAt === undefined ? <NoteActionsDialog node={selected} nodes={nodes} onAdd={(type, parentId) => startCreation({ type, parentId })} onMove={doMove} onRename={() => setNodeEditor({ mode: "rename", node: selected })} onRequestClose={() => setActionsOpen(false)} onRequestTrash={() => { setActionsOpen(false); setTrashConfirmOpen(true) }} open={actionsOpen} returnFocusTo={actionReturnFocus} /> : null}
