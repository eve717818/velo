...
describe("plan responsive CSS contract", () => {
  it("keeps narrow mobile calendar targets readable and clear of fixed navigation", async () => {
    ...
    expect(css).toMatch(/\.monthDay\s*{[^}]*min-height:\s*44px[^}]*min-width:\s*0/s)
    expect(css).toMatch(/\.monthCounts\s*{[^}]*font-size:\s*12px/s)
    expect(css).toMatch(/\.monthTaskPanel\s*{[^}]*max-height:\s*45vh[^}]*overflow:\s*auto/s)
    expect(css).toMatch(/\.weekGroupScroller\s*{[^}]*overflow-x:\s*auto/s)
    expect(css).toMatch(/\.weekGroupBands\s*{[^}]*min-width:\s*640px/s)
    expect(css).toMatch(/\.monthGroupBands\s*{[^}]*grid-template-columns:\s*repeat\(7, minmax\(0, 1fr\)\)/s)
    expect(css).toMatch(/@media \(min-width: 768px\)/)
    expect(css).toMatch(/@media \(min-width: 768px\) and \(orientation: portrait\)/)
    expect(css).toMatch(/@media \(min-width: 1024px\) and \(orientation: landscape\)/)
  })
})
