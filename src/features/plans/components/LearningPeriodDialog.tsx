...
function LearningPeriodForm({ db, onClose, period, periods }: Omit<LearningPeriodDialogProps, "open">) {
  ...
  async function submit() {
    ...
    setErrors({})
    try {
      if (period) await updateLearningPeriod(db, period.id, values, Date.now())
      else await createLearningPeriod(db, values, Date.now())
      onClose()
    } catch (error) {
      if (error instanceof LearningPeriodValidationError) {
        const conflict = error.field === "startDate" ? conflictingPeriodName(values, periods, period?.id) : undefined
  ...
  return (
    <form aria-busy={saving} className={styles.form} noValidate onSubmit={(event) => { event.preventDefault(); void submit() }}>
      ...
      <div className={styles.actions}><button className={styles.secondaryButton} onClick={onClose} type="button">取消</button><button className={styles.primaryButton} disabled={saving} type="submit">{saving ? "正在保存" : "保存周期"}</button></div>
    </form>
  )
}
